              ...
              {isDemo && (
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Optional — e.g. how the insights dashboard works"
                    rows={3}
...
const PROBLEMS = [
  ...
  { icon: TrendingUp, title: "No visibility until it's too late", body: "There's no way to see who's excited about what until turnout on the night tells you — too late to change anything." },
];
...
const PILLARS = [
  {
    ...
    tone: "cobalt",
    body: "Create and manage events, share the details that matter, and invite your community in one place.",
    points: ["Create and manage events", "Share event details", "Invite community members"],
  },
  {
    icon: Vote,
    title: "Community Collaboration",
    tone: "marigold",
    body: "Turn passive attendees into active participants with a real, structured voice in what happens next.",
    points: ["Vote on event ideas", "Submit suggestions", "Leave event feedback", "Join discussions"],
  },
  {
    icon: LayoutDashboard,
    title: "Insights Dashboard",
    tone: "sage",
    body: "Every vote, rating, and comment rolls up into one dashboard organisers can actually act on.",
    points: ["Community sentiment", "Popular ideas", "Event ratings", "Engagement metrics"],
  },
];
...
const STEPS = [
  ...
  { n: "03", icon: BarChart3, title: "Learn", body: "Every vote, rating, and comment feeds the insights dashboard, so the next event starts smarter than the last." },
];
...
const ORG_BENEFITS = [
  ...
  { icon: BarChart3, title: "A dashboard, not a spreadsheet", body: "Sentiment, top requests, ratings, and engagement in one live view — no manually tallying survey forms." },
];
...
const FAQS = [
  ...
  { q: "Does Circlo replace how we already sell tickets or manage RSVPs?", a: "No — Circlo focuses on the planning conversation and the insights behind it. It's built to sit alongside whatever ticketing or RSVP tool you already use." },
...
const SENTIMENT_DATA = [
  { event: "Games Night", score: 68 },
  { event: "Formal", score: 74 },
  { event: "Sports Day", score: 71 },
  { event: "Quiz Night", score: 82 },
  { event: "Guest Talk", score: 79 },
  { event: "Rooftop Social", score: 91 },
];

const TOP_REQUESTS = [
  { name: "Sunset Kayak Trip", votes: 84 },
  { name: "Games Night Rematch", votes: 51 },
  { name: "Alumni Panel", votes: 33 },
  { name: "Charity 5K", votes: 27 },
];

/* ---------------- Insights Dashboard section ---------------- */
function InsightsDashboard() {
  return (
    <div className="bg-white rounded-2xl border border-[#E4E1D6] p-5 sm:p-7 shadow-[0_20px_50px_rgba(20,32,27,0.08)]">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-[#14201B] flex items-center justify-center">
            <LayoutDashboard size={15} className="text-[#F4F5F0]" />
          </div>
          <span className="text-[14.5px] font-semibold" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
            Organiser Dashboard
          </span>
        </div>
      ...
      <div className="grid sm:grid-cols-4 gap-3 mb-5">
        {[
          { label: "Avg. rating", value: "4.7", sub: "/ 5", icon: Star },
          { label: "Sentiment", value: "91%", sub: "positive", icon: Smile },
          { label: "Participation", value: "68%", sub: "of members", icon: Activity },
          { label: "Ideas this month", value: "23", sub: "submitted", icon: Lightbulb },
        ].map((s) => (
          <div key={s.label} className="rounded-xl border border-[#E4E1D6] p-4">
            <s.icon size={14} className="text-[#14201B]/35 mb-2" />
            <div className="flex items-baseline gap-1">
              <span className="text-[22px] font-semibold" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{s.value}</span>
              <span className="text-[12px] text-[#14201B]/40">{s.sub}</span>
            </div>
            <div className="text-[12px] text-[#14201B]/45 mt-0.5">{s.label}</div>
          </div>
        ))}
      </div>

      <div className="grid sm:grid-cols-5 gap-4">
        <div className="sm:col-span-3 rounded-xl border border-[#E4E1D6] p-4">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[13px] font-medium text-[#14201B]/70">Community sentiment by event</span>
            <span className="text-[11px] text-[#14201B]/35" style={{ fontFamily: "IBM Plex Mono, monospace" }}>LAST 6 EVENTS</span>
          </div>
          <div className="h-[140px] -ml-3">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={SENTIMENT_DATA} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="sentimentFill" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#2D4FFF" stopOpacity={0.25} />
                    <stop offset="100%" stopColor="#2D4FFF" stopOpacity={0} />
                  </linearGradient>
                </defs>
                <XAxis dataKey="event" tick={{ fontSize: 10, fill: "#14201B99" }} axisLine={false} tickLine={false} />
                <YAxis hide domain={[50, 100]} />
                <Tooltip
                  contentStyle={{ borderRadius: 10, border: "1px solid #E4E1D6", fontSize: 12, fontFamily: "Inter, sans-serif" }}
                  formatter={(v) => [`${v}%`, "Sentiment"]}
                />
                <Area type="monotone" dataKey="score" stroke="#2D4FFF" strokeWidth={2} fill="url(#sentimentFill)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        <div className="sm:col-span-2 rounded-xl border border-[#E4E1D6] p-4">
          <div className="text-[13px] font-medium text-[#14201B]/70 mb-3">Top requested ideas</div>
          <div className="h-[140px] -ml-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={TOP_REQUESTS} layout="vertical" margin={{ top: 0, right: 10, left: 0, bottom: 0 }}>
                <XAxis type="number" hide />
                <YAxis
                  type="category"
                  dataKey="name"
                  tick={{ fontSize: 9.5, fill: "#14201B99" }}
                  axisLine={false}
                  tickLine={false}
                  width={92}
                />
                <Tooltip
                  ...
                  formatter={(v) => [v, "Votes"]}
                />
                <Bar dataKey="votes" fill="#FFB648" radius={[0, 6, 6, 0]} barSize={14} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
...
export default function CirloLanding() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="max-w-[1120px] mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-7 text-[14px] text-[#14201B]/65">
            ...
            <button onClick={() => scrollTo("dashboard")} className="hover:text-[#14201B] transition-colors">Insights</button>
      ...
      <section className="max-w-[1120px] mx-auto px-5 sm:px-8 pt-14 sm:pt-20 pb-8 grid lg:grid-cols-2 gap-10 items-center">
        <Reveal>
          ...
          <h1
            ...
            style={{ fontFamily: "Space Grotesk, sans-serif" }}
          >
            Community-powered <span className="text-[#2D4FFF]">event planning.</span>
          </h1>
          <p className="text-[16.5px] sm:text-[17.5px] text-[#14201B]/65 leading-relaxed max-w-[480px] mb-8">
            Circlo helps clubs, societies, and brands collect ideas, votes, and feedback in one place — then turns that activity into an insights dashboard organisers actually use to plan the next event. Every event becomes smarter than the last.
          </p>
      ...
      <section className="max-w-[1120px] mx-auto px-5 sm:px-8 py-20 sm:py-28">
        <Reveal>
          ...
          <h2 className="text-[28px] sm:text-[36px] font-semibold tracking-tight max-w-[560px] mb-4" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
            Most events are planned in isolation, then hope for the best.
          </h2>
          <p className="text-[15.5px] text-[#14201B]/60 max-w-[540px] mb-12 leading-relaxed">
            Organisers make decisions without knowing what their community actually wants — and the feedback that could fix that gets lost.
          </p>
        </Reveal>
      ...
      <section id="solution" className="bg-white border-y border-[#E4E1D6]">
        <div className="max-w-[1120px] mx-auto px-5 sm:px-8 py-20 sm:py-28">
          <Reveal>
            ...
            <h2 className="text-[28px] sm:text-[36px] font-semibold tracking-tight max-w-[620px] mb-4" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
              Everything communities need to plan better events, together.
            </h2>
            <p className="text-[15.5px] text-[#14201B]/60 max-w-[560px] mb-16 leading-relaxed">
              Three connected pillars — running events, collecting community input, and turning that input into decisions.
            </p>
          </Reveal>
          ...
          <div className="grid sm:grid-cols-3 gap-6 mb-20">
            {PILLARS.map((p, i) => (
              <Reveal key={p.title} delay={i * 110}>
                <div className="rounded-2xl border border-[#E4E1D6] p-6 h-full bg-[#F4F5F0]">
                  <div
                    className={`w-11 h-11 rounded-xl flex items-center justify-center mb-4 ${
                      p.tone === "cobalt" ? "bg-[#2D4FFF]/10" : p.tone === "marigold" ? "bg-[#FFB648]/20" : "bg-[#6B8F71]/15"
                    }`}
                  >
                    <p.icon
                      size={19}
                      className={p.tone === "cobalt" ? "text-[#2D4FFF]" : p.tone === "marigold" ? "text-[#8A5A00]" : "text-[#3E5C43]"}
                    />
                  </div>
                  <h3 className="text-[17px] font-semibold mb-2" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{p.title}</h3>
                  <p className="text-[13.5px] text-[#14201B]/55 leading-relaxed mb-4">{p.body}</p>
                  <ul className="space-y-1.5">
                    {p.points.map((pt) => (
                      <li key={pt} className="flex items-center gap-2 text-[13px] text-[#14201B]/70">
                        <Check size={13} className="text-[#3E5C43] shrink-0" /> {pt}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div id="how" className="grid sm:grid-cols-3 gap-8">
            {STEPS.map((s, i) => (
      ...
      {/* INSIGHTS DASHBOARD — flagship section */}
      <section id="dashboard" className="max-w-[1120px] mx-auto px-5 sm:px-8 py-20 sm:py-28">
        <Reveal>
          <SectionEyebrow>Insights dashboard</SectionEyebrow>
          <h2 className="text-[28px] sm:text-[36px] font-semibold tracking-tight max-w-[600px] mb-4" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
            The data your community was already giving you — finally usable.
          </h2>
          <p className="text-[15.5px] text-[#14201B]/60 max-w-[560px] mb-12 leading-relaxed">
            Every vote, rating, and comment rolls up automatically into a live dashboard: sentiment trends, the ideas your community wants most, and how engaged they really are — no spreadsheets, no manual tallying.
          </p>
        </Reveal>
        <Reveal delay={100}>
          <InsightsDashboard />
          ...
          <Reveal delay={100}>
            <div className="max-w-[420px] bg-[#F4F5F0] rounded-2xl border border-[#E4E1D6] p-8">
              ...
              <ul className="space-y-2.5 mb-7">
                {["Unlimited idea submissions", "Unlimited community voting", "Full insights dashboard"].map((f) => (
      ...
      <footer className="border-t border-[#E4E1D6]">
        <div className="max-w-[1120px] mx-auto px-5 sm:px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <p className="text-[13px] text-[#14201B]/45">Community-powered event planning.</p>
          <div className="flex items-center gap-1.5 text-[13px] text-[#14201B]/45">
            <Mail size={13} /> hello@circloevents.com
          </div>
        </div>
      </footer>
